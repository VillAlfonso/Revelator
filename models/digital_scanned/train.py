...
Model: Digital: Scanned Document Manipulation
Type: digital_scanned

This trains a BINARY CLASSIFIER to detect: Document scanned, digitally edited, then re-printed/re-scanned to hide manipulat...
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the digital_scanned detection model"""
    ...
    print(f"\n🚀 Training: Digital: Scanned Document Manipulation")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "digital_scanned" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No digital_scanned forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Digital: Scanned Document Manipulation Training")
