...
Model: Carbon Transfer Tracing
Type: traced_carbon

This trains a BINARY CLASSIFIER to detect: Forgery created by placing carbon paper between original and blank document, the...
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the traced_carbon detection model"""
    ...
    print(f"\n🚀 Training: Carbon Transfer Tracing")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "traced_carbon" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No traced_carbon forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Carbon Transfer Tracing Training")
