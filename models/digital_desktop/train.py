...
Model: Digital: Desktop Publishing
Type: digital_desktop

This trains a BINARY CLASSIFIER to detect: Entirely fake document created from scratch using software (Photoshop, Word, etc...
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the digital_desktop detection model"""
    ...
    print(f"\n🚀 Training: Digital: Desktop Publishing")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "digital_desktop" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No digital_desktop forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Digital: Desktop Publishing Training")
