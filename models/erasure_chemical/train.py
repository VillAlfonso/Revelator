...
Model: Erasure: Chemical
Type: erasure_chemical

This trains a BINARY CLASSIFIER to detect: Original text removed using chemical bleaching agents or solvents....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the erasure_chemical detection model"""
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "erasure_chemical" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No erasure_chemical forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Erasure: Chemical Training")
