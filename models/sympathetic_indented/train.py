...
Model: Sympathetic: Indented Writing
Type: sympathetic_indented

This trains a BINARY CLASSIFIER to detect: Hidden writing revealed through indentations left by writing on pages above....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the sympathetic_indented detection model"""
    ...
    print(f"\n🚀 Training: Sympathetic: Indented Writing")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "sympathetic_indented" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No sympathetic_indented forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sympathetic: Indented Writing Training")
