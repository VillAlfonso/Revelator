...
Model: Addition: Insertion
Type: addition_insertion

This trains a BINARY CLASSIFIER to detect: Text or characters inserted into existing spaces within a document....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the addition_insertion detection model"""
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "addition_insertion" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No addition_insertion forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Addition: Insertion Training")
