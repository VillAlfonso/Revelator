...
Model: Obliteration: Ink Stroke
Type: obliteration_ink

This trains a BINARY CLASSIFIER to detect: Original content covered with heavy ink strokes or scribbles....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the obliteration_ink detection model"""
    ...
    print(f"\n🚀 Training: Obliteration: Ink Stroke")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "obliteration_ink" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No obliteration_ink forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Obliteration: Ink Stroke Training")
