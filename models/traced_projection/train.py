...
Model: Projection Process Tracing
Type: traced_projection

This trains a BINARY CLASSIFIER to detect: Forgery created using light box or projector to trace original onto new document...
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the traced_projection detection model"""
    ...
    print(f"\n🚀 Training: Projection Process Tracing")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "traced_projection" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No traced_projection forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Projection Process Tracing Training")
