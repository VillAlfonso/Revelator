...
Model: Indentation / Canal Light Tracing
Type: traced_indentation

This trains a BINARY CLASSIFIER to detect: Forgery where original was placed on top and traced with pressure, leaving groov...
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the traced_indentation detection model"""
    ...
    print(f"\n🚀 Training: Indentation / Canal Light Tracing")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "traced_indentation" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No traced_indentation forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Indentation / Canal Light Tracing Training")
