...
Model: Addition: Interlineation
Type: addition_interlineation

This trains a BINARY CLASSIFIER to detect: Text added between existing lines of writing....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the addition_interlineation detection model"""
    ...
    print(f"\n🚀 Training: Addition: Interlineation")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "addition_interlineation" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No addition_interlineation forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Addition: Interlineation Training")
