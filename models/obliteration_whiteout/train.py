...
Model: Obliteration: White Out / Correction Fluid
Type: obliteration_whiteout

This trains a BINARY CLASSIFIER to detect: Original content covered with white correction fluid (Wite-Out, Liquid Paper)....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the obliteration_whiteout detection model"""
    ...
    print(f"\n🚀 Training: Obliteration: White Out / Correction Fluid")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "obliteration_whiteout" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No obliteration_whiteout forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Obliteration: White Out / Correction Fluid Training")
