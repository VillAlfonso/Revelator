...
Model: Obliteration: Opaque Pigment
Type: obliteration_pigment

This trains a BINARY CLASSIFIER to detect: Original content covered with paint, heavy marker, or other opaque pigment....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the obliteration_pigment detection model"""
    ...
    print(f"\n🚀 Training: Obliteration: Opaque Pigment")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "obliteration_pigment" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No obliteration_pigment forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Obliteration: Opaque Pigment Training")
