...
Model: Sympathetic: Special Ink (Invisible Ink)
Type: sympathetic_special

This trains a BINARY CLASSIFIER to detect: Text written with invisible ink that requires special conditions to reveal....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the sympathetic_special detection model"""
    ...
    print(f"\n🚀 Training: Sympathetic: Special Ink (Invisible Ink)")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "sympathetic_special" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No sympathetic_special forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sympathetic: Special Ink (Invisible Ink) Training")
