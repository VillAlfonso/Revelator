...
Model: Digital: Cut and Paste (Splicing)
Type: digital_cut_paste

This trains a BINARY CLASSIFIER to detect: Elements copied from one digital image and pasted into another....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the digital_cut_paste detection model"""
    ...
    dataset_dir = Path(__file__).parent / "train" / "images"
    ...
    print(f"\n🚀 Training: Digital: Cut and Paste (Splicing)")
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "digital_cut_paste" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No digital_cut_paste forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Digital: Cut and Paste (Splicing) Training")
