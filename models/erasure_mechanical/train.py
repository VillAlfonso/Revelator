...
Model: Erasure: Mechanical
Type: erasure_mechanical

This trains a BINARY CLASSIFIER to detect: Original text removed by physical scraping, rubbing, or eraser....
...
def train(epochs=100, batch=16, device="", imgsz=640):
    """Train the erasure_mechanical detection model"""
    ...
    # Copy best weights
    best_src = weights_dir.parent / "runs" / "erasure_mechanical" / "weights" / "best.pt"
...
def test(weights_path, image_path, conf=0.25):
    ...
    for result in results:
        boxes = result.boxes
        if len(boxes) == 0:
            print("\n✅ No erasure_mechanical forgery detected")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Erasure: Mechanical Training")
