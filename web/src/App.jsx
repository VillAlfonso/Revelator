...
function BootSplash() {
  return (
    <div style={{ padding: 60, textAlign: 'center' }}>
      <div className="mono glow" style={{ color: '#00ff66', fontSize: 12, letterSpacing: 4 }}>
        ▣ INITIALIZING FORENSIC PIPELINE
      </div>
      <div className="mono caret" style={{ color: '#6dba85', fontSize: 11, marginTop: 10, letterSpacing: 2 }}>
        loading
      </div>
    </div>
  );
...
function Layout({ children }) {
  ...
  return (
    <div>
      ...
      <header style={{
        background: 'linear-gradient(180deg, rgba(8,18,12,0.95) 0%, rgba(0,0,0,0.92) 100%)',
        ...
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Link to="/scan" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1 }}>
              <span className="oswald glow-strong" style={{ fontSize: 22, fontWeight: 700, color: '#00ff66', letterSpacing: 6 }}>
                REVELATOR
              </span>
              <span className="mono" style={{ fontSize: 8, letterSpacing: 3, color: '#3f6e4a', marginTop: 3 }}>
                FORENSIC ENGINE
              </span>
            </div>
          </Link>
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 18, lineHeight: 1, textShadow: '0 0 8px rgba(0,255,102,0.5)',
                }}
              >
                {menuOpen ? '✕' : '☰'}
              </button>
          ...
          <div className="nav-mobile-panel" style={{
            ...
            {navItems.map(item => (
              <Link key={item.path} to={item.path}
                style={{ ...linkStyle(location.pathname === item.path), borderBottom: 'none', padding: '12px 16px' }}>
                {item.label}
              </Link>
            ))}
            <button
